...
export default function Playground() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-h3 font-bold text-ink">پلی‌گراند</h1>
      ...
      <div className="flex gap-2">
        <input
          ...
          className="w-full min-h-[44px] rounded-btn border border-sand bg-bone px-4 py-2.5 text-[0.95rem] text-ink focus:border-brass focus:outline-none"
        ...
        <button
          ...
          onClick={run}
          className="shrink-0 rounded-btn bg-brass px-6 py-2.5 text-[0.95rem] font-medium text-white transition-colors hover:bg-brass-dark disabled:opacity-60"
        >
          {pending ? "…" : "اجرا"}
        </button>
      </div>

      {err && <div className="rounded-btn border border-red-500/30 bg-red-500/10 px-4 py-3 text-caption text-red-300">{err}</div>}

      {result && (
        <div className="grid gap-5 lg:grid-cols-2">
          {/* پاسخ */}
          <section className="rounded-card border border-sand bg-pine p-5 shadow-soft">
            <div className="mb-2 flex items-center justify-between">
              <h2 className="font-heading text-body font-semibold text-ink">پاسخ مدل</h2>
          ...
          <section className="rounded-card border border-sand bg-pine p-5 shadow-soft">
            <h2 className="mb-3 font-heading text-body font-semibold text-ink">
            ...
            {result.chunks.length === 0 ? (
              ...
              <div className="space-y-2">
                {result.chunks.map((c, i) => (
                  <div key={i} className="rounded-btn border border-sand bg-bone/60 px-3.5 py-2.5">

        ...
        {STEPS.map((step, i) => {
          ...
          return (
            <Reveal as="li" key={step.no} delay={i * 90} className="relative">
              ...
              {!isLast && (
                <span
                  className="absolute right-[calc(-50%+1.75rem)] top-7 hidden h-px w-full bg-ink/15 lg:block"
                  aria-hidden="true"
                />
              )}
              <div className="relative flex flex-col">
                <span className="relative z-10 inline-flex h-14 w-14 items-center justify-center rounded-full bg-brass text-white shadow-soft">
                  <Icon width={26} height={26} />
                </span>
                <span className="nums mt-5 font-heading text-caption font-semibold text-brass">
                  مرحله‌ی {toFa(step.no)}
                </span>
                <h3 className="mt-1 font-heading text-h3 font-semibold text-ink">
                  {step.title}
                </h3>

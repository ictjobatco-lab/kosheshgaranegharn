...
const STARTERS = [
  "کوشش‌گران قرن دقیقاً چه خدماتی ارائه می‌دهد؟",
  "هوش مصنوعی و دستیار هوشمند چطور به کسب‌وکارم کمک می‌کند؟",
  "هزینه و مدت پروژه‌های نرم‌افزاری چقدر است؟",
    ...
    <div className="flex min-h-dvh flex-col bg-bone">
      <header className="sticky top-0 z-10 border-b border-sand bg-bone/95 backdrop-blur">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-4 px-5 py-3.5">
          <a href="/" className="rounded-btn" aria-label="کوشش‌گران قرن — خانه">
            <Logo />
          </a>
          <a
            href="/#consultation"
            className="inline-flex items-center gap-1.5 rounded-btn bg-brass px-4 py-2 text-caption font-medium text-white transition-colors hover:bg-brass-dark"
          >
            درخواست مشاوره
          </a>
        </div>
      ...
      <div className="border-t border-sand bg-bone">
        <div className="mx-auto max-w-3xl px-5 py-4">
          <div className="flex items-end gap-2 rounded-card border border-sand bg-pine p-2 shadow-soft focus-within:border-brass">
            <textarea
              ref={inputRef}
              value={input}
              ...
              onKeyDown={onKeyDown}
              rows={1}
              placeholder="سؤال‌تان را درباره‌ی خدمات ما بنویسید…"
            ...
            <button
              ...
              className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-btn bg-brass text-white transition-colors hover:bg-brass-dark disabled:opacity-50"
              aria-label="ارسال"
            >
              <SendIcon />
            </button>
...
function WelcomeScreen({ onPick }: { onPick: (q: string) => void }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center py-10 text-center">
      <span className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-brass text-white">
        <SparkIcon />
      </span>
      <h1 className="mt-5 font-heading text-h3 font-bold text-ink">دستیار هوشمند کوشش‌گران قرن</h1>
      <p className="mt-2 max-w-md text-body text-slate">
        درباره‌ی خدمات و مسیر همکاری بپرسید. هر وقت آماده بودید، درخواست مشاوره‌ی رایگان ثبت کنید.
      </p>
      <div className="mt-8 grid w-full max-w-lg gap-3 sm:grid-cols-2">
        {STARTERS.map((q) => (
          <button
            ...
            type="button"
            onClick={() => onPick(q)}
            className="rounded-card border border-sand bg-pine px-4 py-3 text-right text-[0.95rem] text-ink shadow-soft transition-colors hover:border-brass/40"
          >
            {q}
          </button>
        ))}
    ...
    <div className={isUser ? "flex justify-start" : "flex justify-end"}>
      <div className={isUser ? "max-w-[85%]" : "w-full max-w-[92%]"}>
        <div
          className={
            isUser
              ? "rounded-card rounded-tr-sm bg-brass px-4 py-3 text-[0.95rem] leading-7 text-white"
              : msg.error
                ? "rounded-card border border-red-500/30 bg-red-500/10 px-4 py-3 text-[0.95rem] leading-7 text-red-300"
                : "rounded-card rounded-tl-sm border border-sand bg-pine px-4 py-3 text-[0.95rem] leading-8 text-ink shadow-soft"
          }
        >
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-2 px-1">
      ...
      {unique.map((s, i) => (
        <span
          key={i}
          className="inline-flex items-center gap-1 rounded-full bg-sand px-2.5 py-0.5 text-[0.75rem] text-brass"
...
function TypingIndicator() {
  return (
    <div className="flex justify-end">
      <div className="rounded-card rounded-tl-sm border border-sand bg-pine px-4 py-3.5 shadow-soft">

...
export default function WidgetChat({
  ...
  return (
    <div className="flex h-dvh flex-col bg-bone">
      {/* هدر فشرده */}
      <header className="flex items-center gap-2.5 px-4 py-3 text-white" style={{ backgroundColor: primaryColor }}>
        <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/15">
          <SparkIcon />
        </span>
        <div className="leading-tight">
          <p className="font-heading text-[0.95rem] font-bold">دستیار کوشش‌گران قرن</p>
      ...
      <div ref={scrollRef} className="flex-1 space-y-3.5 overflow-y-auto px-4 py-4">
        ...
        {empty && (
          <p className="px-1 pt-1 text-[0.75rem] text-slate">
            می‌توانید درباره‌ی خدمات، قیمت‌ها یا فرایند همکاری بپرسید.
          </p>
        )}
      ...
      <div className="border-t border-sand bg-bone px-3 pb-3 pt-2">
        ...
        <div className="flex items-end gap-2 rounded-card border border-sand bg-pine p-1.5">
          <textarea
            value={input}
          ...
          <button
            ...
            className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-btn text-white transition-opacity disabled:opacity-50"
            style={{ backgroundColor: primaryColor }}
            aria-label="ارسال"
          >
            <SendIcon />
  ...
  if (isUser) {
    return (
      <div className="flex justify-start">
        <div
          className="max-w-[85%] rounded-card rounded-tr-sm px-3.5 py-2.5 text-[0.9rem] leading-7 text-white"
          style={{ backgroundColor: primaryColor ?? "#7C6FF0" }}
        >
  ...
  return (
    <div className="flex justify-end">
      <div
        className={
          error
            ? "max-w-[88%] rounded-card border border-red-500/30 bg-red-500/10 px-3.5 py-2.5 text-[0.9rem] leading-7 text-red-300"
            : "max-w-[88%] rounded-card rounded-tl-sm border border-sand bg-pine px-3.5 py-2.5 text-[0.9rem] leading-7 text-ink shadow-soft"
        }
      >
...
function Feedback({ conversationId }: { conversationId: string | null }) {
  ...
  return (
    <div className="mt-1 flex items-center gap-1.5 px-1">
      ...
      <button type="button" onClick={() => rate("up")} className="rounded p-1 text-slate hover:text-brass" aria-label="مفید بود">
        <ThumbIcon up />
      </button>
      <button type="button" onClick={() => rate("down")} className="rounded p-1 text-slate hover:text-brass" aria-label="مفید نبود">
        <ThumbIcon />
      </button>
    </div>
  );
...
function TypingDots() {
  return (
    <div className="flex justify-end">
      <div className="rounded-card rounded-tl-sm border border-sand bg-pine px-3.5 py-3 shadow-soft">

...
export default async function BlogPostPage({ params }: Props) {
  ...
  const articleJsonLd = {
    ...
    author: { "@type": "Organization", name: "کوشش‌گران قرن" },
    publisher: { "@type": "Organization", name: "کوشش‌گران قرن" },
  };
  const faqJsonLd =
    post.faq.length > 0
      ? {
        ...
        <header className="border-b border-sand bg-bone pb-8 pt-32 sm:pt-36">
          <Container className="max-w-3xl">
            <nav className="mb-6 text-caption text-slate" aria-label="مسیر">
              <a href="/blog" className="transition-colors hover:text-brass">
                بلاگ
              </a>
              <span className="mx-2" aria-hidden="true">/</span>
              <span className="text-ink/70">{post.title}</span>
            </nav>
            <h1 className="font-heading text-[1.75rem] font-bold leading-tight text-ink sm:text-h1">
              {post.title}
            </h1>
        ...
        <Container className="max-w-3xl py-12">
          ...
          {post.keywords.length > 0 && (
            <div className="mt-10 flex flex-wrap gap-2 border-t border-sand pt-6">
              {post.keywords.map((k) => (
                <span key={k} className="rounded-full bg-sand px-3 py-1 text-caption text-brass">
                  {k}
                </span>
              ))}
            </div>
          ...
          {/* پرسش‌های متداول */}
          {post.faq.length > 0 && (
            <section className="mt-12 rounded-card border border-sand bg-pine p-7 shadow-soft sm:p-8">
              <h2 className="mb-5 font-heading text-h3 font-semibold text-ink">
                پرسش‌های متداول
              </h2>
              <div className="divide-y divide-sand">
                {post.faq.map((f) => (
                  <details key={f.question} className="group py-4">
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-medium text-ink transition-colors group-open:text-brass [&::-webkit-details-marker]:hidden">
                      {f.question}
                      <span
                        aria-hidden="true"
          ...
          {/* دعوت به مشاوره */}
          <aside className="mt-12 rounded-card bg-brass p-8 text-center text-white shadow-soft sm:p-10">
            <h2 className="font-heading text-h3 font-semibold text-white">
              وضعیت کسب‌وکار شما فرق دارد؟
            </h2>
            <p className="mx-auto mt-3 max-w-md leading-8 text-white/85">
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <a
                href="/#consultation"
                className="inline-flex min-h-[48px] items-center justify-center gap-2 rounded-btn bg-white px-6 font-medium text-brass transition-colors hover:bg-white/90"
              >
                درخواست مشاوره
              </a>
              <a
                href="/consultant"
                className="inline-flex min-h-[48px] items-center justify-center gap-2 rounded-btn border border-white/40 px-6 font-medium text-white transition-colors hover:bg-white/10"
              >
